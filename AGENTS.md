...
**実装済み機能**:
- Spout SDK version 取得
- DirectX 11 初期化疎通テスト
- **Receiver API（受信機能）** - 2026-01-08 実装完了
  - ライフサイクル管理（作成/破棄）
  - 外部デバイスでの初期化
  - テクスチャ受信（receive_into / receive 形式）
  - センダー情報取得（名前・サイズ・フォーマット）
  - 接続状態/更新/新規フレーム判定

## 実行方法

spoutdx-ffi はライブラリであり、単体で実行することはできません。Rust/C/C++ などのホスト言語から FFI 経由で呼び出して使用します。

dev.ps1 スクリプトを使用して、サンプルの Rust バインディングと共にビルド・実行できます：
```powershell
.\dev.ps1
```
